...
const fetchMonthMarks = async () => {
  ...
  const set = new Set();
  data.forEach(it => expandDates(it.startDate, it.endDate).forEach(d => set.add(d)));
  markedSet.value = set;
...
const isMarked = (d) => d && markedSet.value.has(`${year.value}-${fmt2(month.value)}-${fmt2(d)}`);
    ...
    <table class="tbl">
      ...
      <tbody>
        <tr v-for="(row,ri) in matrix" :key="ri">
          <td v-for="(d,ci) in row" :key="ci" class="day"
...
.tbl td{height:70px;text-align:center}
.day{cursor:pointer}
.num{display:flex;align-items:center;justify-content:center;width:48px;height:48px;border-radius:100%;margin:auto;font-weight:700}
.mark{background:#BFEAFF}
.today{color:steelblue}
.sun{color:tomato}
.legend{margin-top:6px;font-size:12px;color:#777;display:flex;gap:6px;align-items:center}
.dot{width:8px;height:8px;border-radius:50%;background:green;display:inline-block}
</style>